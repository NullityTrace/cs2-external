...
bool pProcess::AttachProcessHj(const char* ProcessName)
{
	...
	if (pid_)
	{
		...
		{
			std::cout << "[cheat] Handle Hijack failed. No fallback allowed." << std::endl;
			return false;
		}

		if (!EnumProcessModulesEx(this->handle_, modules, sizeof(modules), &_, LIST_MODULES_64BIT)) {
			std::cerr << "[cheat] Failed to enumerate modules." << std::endl;
			CloseHandle(handle_);
			return false;
		}
		...
		if (!GetModuleInformation(this->handle_, modules[0], &module_info, sizeof(module_info))) {
			std::cerr << "[cheat] Failed to get module information." << std::endl;
			CloseHandle(handle_);
			return false;
		}
		base_client_.size = module_info.SizeOfImage;
